...
export async function GET(req: NextRequest, ctx: Ctx) {
  ...
  try {
    const rows = await query<{ ID: string; NAME: string; STATUS: string; FLOW_JSON: string; CREATED_AT: string; UPDATED_AT: string }>(
      `SELECT ID, NAME, STATUS, FLOW_JSON, CREATED_AT, UPDATED_AT FROM "HATCH"."SEQUENCES" WHERE ID = ? AND OWNER_UID = ?`,
      [id, uid],
    );
...
export async function PUT(req: NextRequest, ctx: Ctx) {
  ...
  try {
    await query(
      `UPDATE "HATCH"."SEQUENCES" SET NAME = COALESCE(?, NAME),
    ...
    console.error('[PUT /api/sequences/[id]]', e);
    return NextResponse.json({ message: 'Database error' }, { status: 500 });
  }
}
...
export async function DELETE(req: NextRequest, ctx: Ctx) {
  ...
  const { id } = await ctx.params;
  try {
    await query(`DELETE FROM "HATCH"."SEQUENCES" WHERE ID = ? AND OWNER_UID = ?`, [id, uid]);
    return NextResponse.json({ ok: true });
  } catch (e) {
